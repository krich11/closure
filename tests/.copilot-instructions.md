...
- **Never pipe test output through `tail` or `head`** — always show full output so failures are visible.
- The user will tell you when a full or slow test run is warranted. Default to fast, focused validation.

## Headless Mode

Tests run in Chrome's `--headless=new` mode by default. This is a full browser that supports extensions and service workers — no visible windows, no focus-stealing. To run with visible browser windows for debugging, use `npm run test:headed`.

## Worker-Scoped Chrome Instances

Chrome launches **once per worker** (not per test). The `context` and `extensionId` fixtures are worker-scoped for speed. An auto-running `_cleanup` fixture resets state between tests:

1. Closes all pages from the previous test (keeps one alive for the context)
2. Navigates the surviving page to `offscreen/offscreen.html` (minimal — no side effects)
3. Clears and re-seeds `chrome.storage.local` with `DEFAULT_STORAGE`

**Important constraints:**
- Never park cleanup pages on `about:blank` — the Dead End Sweeper detects and closes blank tabs
- Never call `chrome.alarms.clearAll()` in cleanup — alarm-checking tests depend on `onInstalled` alarms persisting
- Tests that need specific storage state should seed it themselves via `page.evaluate()`
- The `DEFAULT_STORAGE` object in fixtures.js must stay in sync with background.js `DEFAULT_STORAGE`
...
|---|---|
| `npm test` | Default suite — all tests **except** `@slow` (~97 tests, ~22s) |
